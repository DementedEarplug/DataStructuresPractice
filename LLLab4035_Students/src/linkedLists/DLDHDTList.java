...
public class DLDHDTList<E> implements LinkedList<E>
{
	...
	public void addNodeBefore(Node<E> target, Node<E> nuevo)
	{
			...
			DNode<E> before = dTarget.getPrev();

			before.setNext(dNuevo);
			dTarget.setPrev(dNuevo);
			dNuevo.setNext(dTarget);
			dNuevo.setPrev(before);
	...
	{
		if (length() == 0)
			throw new NodeOutOfBoundsException("getAfterNode(...) : target is the first node.");
		DNode<E> dTarget = (DNode<E>) target;
	...
	public void removeNode(Node<E> target)
	{
		 ...
		 DNode<E> dTarget = (DNode<E>) target;
		 DNode<E> prev = dTarget.getPrev();
		 DNode<E> next = dTarget.getNext();
	...
	private static class DNode<E> implements Node<E>
	{
		...
		/**
		 * Just set references prev and next to null. Disconnect the node from
		 * the linked list....
		 */
