 ...
 *
 */
public class NodePositionList2<T> implements PositionList<T>, PositionListIteratorMaker<T>
{
	...
	{
		NodePositionList2<Position<T>> pList = new NodePositionList2<Position<T>>();
		try
		{
			if (!this.isEmpty())
			{
	...
	public Iterator<T> makeIterator(PositionList<T> pl)
	{
		...
		while (input != 1 || input != 2)
		{
			...
			else if (input == 2)
			{
				...
				System.out.println();
				in.close();
				return new PositionListElementsBackWardsIterator<T>(pl);
			} 
			else
	...
	private static class DNode<T2> implements Position<T2>
	{
		...
		private NodePositionList2<T2> myList; // list it belongs to

		public DNode(T2 e, DNode<T2> p, DNode<T2> n, NodePositionList2<T2> list)
		{
			element = e;
			next = n;
			prev = p;
		...
		public void setList(NodePositionList2<T2> n)
		{
			myList = n;
		}

		public NodePositionList2<T2> getList()
		{
			return myList;
		}
